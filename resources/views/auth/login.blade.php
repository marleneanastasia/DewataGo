    ...
    <div class="floating-card relative z-10 w-full max-w-sm mx-4">
        ...
        <div class="relative rounded-xl border border-white/10 bg-[#04120b] p-8 shadow-2xl">
            ...
            <div class="mb-6 flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-lg font-semibold text-white">Masuk</h1>
                    <p class="text-xs text-white/50">Selamat datang kembali</p>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-3">
                ...
                @if (session('status'))
                    <div class="rounded-lg bg-lime-400/10 border border-lime-400/20 px-3 py-2 text-xs text-lime-300">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <div class="flex items-center justify-between text-xs">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="remember"
                               class="h-3.5 w-3.5 rounded border-white/20 bg-white/5 text-lime-400 focus:ring-lime-400">
                        <span class="text-white/60">Ingat saya</span>
                    </label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-lime-400 hover:text-lime-300 transition">Lupa kata sandi?</a>
                    @endif
                </div>
            ...
            <p class="text-center text-xs text-white/50 mt-5">
                Belum punya akun?
                <a href="{{ route('register') }}" class="text-lime-400 hover:text-lime-300 font-medium">Daftar</a>
            </p>
        </div>
    </div>
