...
<div class="mx-auto max-w-4xl">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="font-display text-2xl font-semibold">Tambah Destinasi Baru</h2>
            <p class="text-sm text-white/50">Lengkapi informasi destinasi wisata berikut.</p>
        </div>
        <a href="{{ route('destinasi.index') }}" class="text-sm text-white/60 transition hover:text-lime-300">← Kembali</a>
    </div>

    <form method="POST" action="{{ route('destinasi.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama') }}" required placeholder="Contoh: Terasering Tegallalang"
                ...
                <div>
                    ...
                    <select name="kategori_wisata_id" required
                        ...
                        @foreach ($kategoriWisatas as $kategori)
                            <option value="{{ $kategori->id }}" @selected(old('kategori_wisata_id') == $kategori->id)>{{ $kategori->nama }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mt-4">
                ...
                <input type="text" name="lokasi" value="{{ old('lokasi') }}" required placeholder="Contoh: Ubud, Gianyar"
                       class="w-full rounded-lg border border-white/15 bg-white/10 px-4 py-2.5 text-sm text-white placeholder-white/40 outline-none transition focus:border-lime-300/60 focus:ring-2 focus:ring-lime-300/30">
            ...
            <div class="mt-4">
                ...
                <textarea name="deskripsi" rows="4" required placeholder="Jelaskan tentang destinasi ini..."
                          class="w-full rounded-lg border border-white/15 bg-white/10 px-4 py-2.5 text-sm text-white placeholder-white/40 outline-none transition focus:border-lime-300/60 focus:ring-2 focus:ring-lime-300/30">{{ old('deskripsi') }}</textarea>
        ...
        <div class="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                ...
                <div>
                    ...
                    <input type="number" name="diskon" value="{{ old('diskon', 0) }}" min="0" max="90" placeholder="0"
                           class="w-full rounded-lg border border-white/15 bg-white/10 px-4 py-2.5 text-sm text-white placeholder-white/40 outline-none transition focus:border-lime-300/60 focus:ring-2 focus:ring-lime-300/30">
                ...
                <div>
                    ...
                    <label class="flex cursor-pointer items-center gap-2 pt-2">
                        <input type="checkbox" name="unggulan" value="1" @checked(old('unggulan'))
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    ...
                    <div id="panel-upload" class="hidden">
                        ...
                        {{-- Kotak Upload (Akan disembunyikan kalau sudah pilih foto) --}}
                        <div id="upload-box">
                            ...
                            <label for="gambarInput"
                                ...
                                <span class="text-sm text-white/60">Klik untuk pilih gambar</span>
                                <span class="mt-1 text-xs text-white/40">JPG / PNG • Maksimal 2MB</span>
                            </label>
                            @error('gambar') <p class="mt-2 text-xs text-red-400">{{ $message }}</p> @enderror
                        </div>

                        {{-- Info File Terpilih (Muncul setelah pilih foto) --}}
                ...
                <div>
                    ...
                    <div class="h-48 overflow-hidden rounded-xl border border-white/10 bg-black/30">
                        <img id="imagePreview"
                             ...
                             alt="Preview" class="h-full w-full object-cover opacity-60 transition-opacity duration-300">
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function resetFileInput() {
        ...
        // Kembalikan preview ke default (transparan)
        const img = document.getElementById('imagePreview');
        img.src = PLACEHOLDER;
